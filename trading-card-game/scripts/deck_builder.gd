...
func _ready():
	...
	UI.scale = Vector2(screenSize.end[0]/(UI.size.x + 200), screenSize.end[0]/(UI.size.x + 200))
	...
	UI.find_child("ScrollContainer").custom_minimum_size.y = (screenSize.end[1] - (UI.find_child("GridContainer").size.y/1.7 + 12 + UI.find_child("Confirm").size.y + 12 + 6)*UI.scale.y)/UI.scale.y
	...
	for i in range(len(Player.cards)):
		if i >= collectedCardsContainer.get_child_count():
			...
			if UI.find_child("GridContainer"):
				...
				tempButton.add_theme_stylebox_override("normal", load("res://assets/styleBox/deckSelectionNotSelected.tres"))
				tempButton.add_theme_stylebox_override("hover", load("res://assets/styleBox/deckSelectionNotSelected.tres"))
				tempButton.add_theme_stylebox_override("pressed", load("res://assets/styleBox/deckSelectionNotSelected.tres"))
